"""
Fix v13 — AB, AC, AD, AE, AF:
AB: buildTitleFilters com fade-in/out alpha 0.4s (igual versículo)
AC: Sombra longa no karaoke (shadowx/shadowy=3, black@0.8) nas 2 camadas
AD: Transições xfade variadas por cena (fade/wipeleft/slideleft/wiperight/slideright)
AE: Watermark alterna canto (top-right ou top-left) baseado no videoId
AF: ctaText rotativo de 5 frases quando for o texto padrão
"""
...
// Fix AD: pool de transições — sorteia uma por corte
...
if (N === 1) {
  ...
  for (var xi = 0; xi < N - 1; xi++) {
    ...
    // Fix AD: transição aleatória por corte
...
// ── FIX M+Q+T+AC: KARAOKE 2 CAMADAS + SOMBRA LONGA ───────────────────────────
...
// Fix AC: shadowx=3:shadowy=3:shadowcolor=black@0.8 nas 2 camadas
function buildSubtitleFilters(text, fontP, narDur, stopAt) {
  ...
  for (var gi = 0; gi < words.length; gi += GRP_SIZE) {
    ...
    // Layer 1: grupo em branco semi-transparente + Fix AC sombra
    filters.push(
    ...
    // Layer 2: palavra individual laranja/amarelo + Fix AC sombra
...
// ── FIX S+U: VERSICULO CENTRALIZADO 2s COM FADE-IN/OUT ───────────────────────
...
// ── FIX W+AB: TÍTULO CENTRALIZADO 2s COM FADE-IN/OUT ─────────────────────────
...
function buildTitleFilters(text, fontP) {
  ...
  // Fix AB: alpha fade-in 0.4s / hold / fade-out 0.4s dentro da janela [0, 2.0]
  var FADE_T = 0.4;
  var DUR_T  = 2.0;
  var titleAlpha =
    'if(lt(t,' + FADE_T + '),' +
      'clip(t/' + FADE_T + ',0,1),' +
      'if(gt(t,' + (DUR_T - FADE_T) + '),' +
        'clip((' + DUR_T + '-t)/' + FADE_T + ',0,1),' +
        '1))';
...
// Branding + Fix AE: watermark alterna canto baseado em videoId
...
if (!hasLogo) {
  // Fix AE: se charCode[0] do videoId é par → canto direito; ímpar → canto esquerdo
  var wmRight = ((videoId || '').charCodeAt(0) || 0) % 2 === 0;
  var wmXPos  = wmRight ? 'w-text_w-20' : '20';
  // Fix O: watermark pulsante
  allVfFilters.push(
...
// Fix N: fade-in do preto PRIMEIRO
...
// Fix R: fadeOutFilter fora do vfChain
...
print("Patching FFmpeg: Assemble Video (v13 — AB/AC/AD/AE/AF)...")
...
if st == 200:
    print()
    print("=== DONE v13 ===")
    print("FIX AB: buildTitleFilters com alpha fade-in/out 0.4s (igual ao versículo)")
    print("FIX AC: shadowx=3:shadowy=3:shadowcolor=black@0.8 nas 2 camadas do karaoke")
    print("FIX AD: xfade aleatório por corte — fade/wipeleft/slideleft/wiperight/slideright")
    print("FIX AE: watermark alterna top-right/top-left baseado em videoId.charCodeAt(0)%2")
    print("FIX AF: ctaText rotativo de 5 frases quando for o texto padrão (hash de videoId)")
