...
export default function CelulaPage() {
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    setCreating(false);
    if (!res.ok) { setCreateError(data.error ?? "Erro ao criar célula"); return; }
    // Recarregar dados
      ...
      <div className="w-full max-w-5xl mx-auto px-4 sm:px-6 md:px-10 py-8 md:py-14">
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="mb-8 text-center">
          ...
          <div className="flex items-center justify-center gap-2 mt-2">
            {group.isPrivate ? (
              <span className="flex items-center gap-1 text-sm text-slate-500"><Lock className="w-3.5 h-3.5" /> Célula privada</span>
            ) : (
              <span className="flex items-center gap-1 text-sm text-slate-500"><Globe className="w-3.5 h-3.5" /> Célula pública</span>
            )}
          </div>
        </motion.div>
  ...
  return (
    <div className="w-full px-6 md:px-10 py-10 md:py-14">
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="mb-10 text-center">
        <p className="text-sm font-bold uppercase tracking-[0.3em] text-gold-dark/60 mb-1">Células</p>
        <h1 className="font-serif text-4xl md:text-5xl font-bold text-slate-900 tracking-tight">
          Encontre sua célula
        </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto">
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="divine-card p-5 sm:p-6 md:p-8 flex flex-col gap-6">
          ...
          <form onSubmit={handleCreate} className="flex flex-col gap-4">
            <div>
              <label className="text-base font-semibold text-slate-700 mb-2 block">Nome da célula</label>
              <input
                type="text"
                value={name}
                onChange={(e) => setName(e.target.value.slice(0, 60))}
                maxLength={60}
                placeholder="Ex: Célula Esperança, Grupo Família..."
            ...
            {isPremium ? (
              <div className="flex items-center justify-between p-4 rounded-xl bg-divine-50 border border-divine-200">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-base font-semibold text-slate-700">{isPrivate ? "Célula privada" : "Célula pública"}</p>
              ...
              <div className="flex items-center gap-3 p-4 rounded-xl bg-divine-50 border border-divine-200">
                ...
                <div>
                  <p className="text-sm font-semibold text-slate-600">Célula pública</p>
            ...
            >
              {creating ? (
                ...
                <><Plus className="w-5 h-5" /> Criar célula</>
              )}
            </button>
          </form>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="flex flex-col gap-4">
          <div className="text-center">
            <h2 className="font-serif text-2xl font-bold text-slate-800">Células abertas</h2>
          ...
          {publicGroups.length === 0 ? (
            <div className="divine-card p-8 flex flex-col items-center gap-3 text-center">
              ...
              <p className="text-base text-slate-500">Nenhuma célula pública ainda.</p>
