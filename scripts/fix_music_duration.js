...
async function main() {
  ...
  // 1. Adicionar calculo de totalDuration antes de "Build elements"
  const buildMarker = '// Build elements';
  if (!code.includes('var totalDuration')) {
    code = code.replace(
      buildMarker,
      'var totalDuration = scenes.reduce(function(s, c) { return s + (c.duracao || 6); }, 0);\n\n' + buildMarker
    );
    console.log('totalDuration adicionado');
  } else {
    console.log('totalDuration ja existe');
  }

  // 2. Adicionar duration na musica
  const oldMusic = [
    '    source: musicUrl,',
    '    volume: \'12%\',',
    '    loop: true,',
  ...
  if (code.includes(oldMusic)) {
    code = code.replace(oldMusic, newMusic);
    console.log('duration adicionado na musica');
  } else if (code.includes('duration: totalDuration')) {
    console.log('duration ja existe na musica');
  } else {
    console.log('AVISO: nao encontrou o trecho da musica — verifique manualmente');
  }
